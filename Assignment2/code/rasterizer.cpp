...
{   
    ...
    float ao_x = x - A.x();
    float ao_y = y - A.y();
    float judge1 = ab_x*ao_y - ao_x*ab_y;

    float judge2 = (C.x()-B.x())*(y-B.y()) - (x-B.x())*(C.y()-B.y());

    float judge3 = (A.x()-C.x())*(y-C.y()) - (x-C.x())*(A.y()-C.y());
...
void rst::rasterizer::rasterize_triangle(const Triangle& t) {
    ...
    for(int i=arr_x[0]; i<=arr_x[2]; i++) {
        for(int j=arr_y[0]; j<=arr_y[2]; j++) {
